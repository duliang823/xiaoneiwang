...
var xmlrequest;
function createXMLHttpRequest() {   //根据不同的浏览器创建不同的ajax引擎
...
function setProUnis(o) {
	...
	createXMLHttpRequest();   //创建xmlrequest
	if (xmlrequest) {   //if (xmlrequest)表示xmlrequest创建成功
		
		//准备发送请求         
		var req = "/xiaoneinew/ChangeInfo?proid="+o.id+"&cid="+o.coId;  //proid省的id，cid国家的id
...
//		window.alert(req);
		...
		xmlrequest.open("get", req, true);
		//指定处理结果的函数
		xmlrequest.onreadystatechange = getProUnis;    //响应返回的结果
		//发送请求
		xmlrequest.send();
	}
}
...
//	document.getElementById("university").value=o.innerHTML;
	$("#university").val(o.innerHTML);  //把选中的大学填入输入框中
	//把选择大学的框框关闭
	//document.getElementById("uniDiv").style.display="none";
	$("#uniDiv").css("display","none");   //关闭大学的选择框
	//document.getElementById("uuniversity").value=o.id;
	$("#uuniversity").val(o.id);   //把id号给隐藏框
}
function showUniTbl() {
